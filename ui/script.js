    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://esx-mdt/getWarrants', JSON.stringify({}));
        ...
        closeMDT() {
            $.post('http://esx-mdt/close', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://esx-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://esx-mdt/viewOffender', JSON.stringify({
		...
		evFile(currentid) {
			    $.post('http://esx-mdt/evFile', JSON.stringify({
                id: currentid,
            }));
		},
        SaveOffenderChanges() {
            $.post('http://esx-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://esx-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://esx-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://esx-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://esx-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://esx-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://esx-mdt/vehicleSearch', JSON.stringify({
        ...
        VOWNSearch() {
            $.post('http://esx-mdt/performVEHSEARCH', JSON.stringify({
                char_id: this.vehicle_selected.char_id
            }));
            
            this.vehicle_results.results = true;
            this.vehicle_results.query = this.vehicle_selected.ownersearch;
            return;
        },
        OpenVehicleDetails(result) {
            $.post('http://esx-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://esx-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://esx-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://esx-mdt/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://esx-mdt/sentencePlayer', JSON.stringify({
...
document.onreadystatechange = () => {
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            ...
            } else if (event.data.type == "returnedVEHmatches") {
                mdtApp.vehicle_results.results = event.data.matches;
                mdtApp.vehicle_selected = {
                    plate: null,
                    type: null,
